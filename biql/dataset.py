...
class BIDSFile:
    ...
    def _parse_entities(self) -> Dict[str, str]:
        ...
        if parent in ['anat', 'func', 'dwi', 'fmap', 'perf', 'meg', 'eeg', 'ieeg', 'beh', 'pet', 'micr', 'nirs', 'motion', 'mrs']:
    ...
    def _is_applicable_metadata(self, json_path: Path, file_entities: Dict[str, str]) -> bool:
    ...
    def _index_files(self) -> List[BIDSFile]:
        ...
        for pattern in patterns:
            for filepath in self.root.glob(pattern):
                ...
                if any(part in ['code', 'derivatives', 'sourcedata'] for part in filepath.parts):
                    continue
