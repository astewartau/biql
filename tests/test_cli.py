...
class TestCLI:
    ...
    def test_format_options(self, synthetic_dataset_path):
        ...
        # Test paths format
        result = self.run_biql_command(
            ["datatype=func", "--format", "paths"], 
            synthetic_dataset_path
        )
        assert result.returncode == 0
        lines = result.stdout.strip().split('\n')
        assert all(line.endswith('.nii') or line.endswith('.gz') for line in lines if line)
    ...
    def test_query_validation(self, synthetic_dataset_path):
        ...
        # Valid query
        result = self.run_biql_command(
            ["--validate-only", "sub=01 AND datatype=func"], 
            synthetic_dataset_path
        )
        assert result.returncode == 0
        ...
        # Invalid query
        result = self.run_biql_command(
            ["--validate-only", "SELECT FROM WHERE"], 
            synthetic_dataset_path
        )
...
class TestCLIEdgeCases:
    ...
    def test_malformed_dataset(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            result = subprocess.run(
                [sys.executable, "-m", "biql.cli", "sub=01", "--dataset", str(dataset_path)],
                capture_output=True,
                text=True,
                cwd=Path(__file__).parent.parent
            )
